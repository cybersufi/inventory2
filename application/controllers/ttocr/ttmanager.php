...
class ttmanager extends CI_Controller {
	...
	public function __construct() {
		...
		$this->index = 'ttocr/tt/index';
		$this->result = 'ttocr/result';
		...
		foreach($config['ttmanager'] as $key => $value) {
			$this->page_config->$key = $value;
		}
	...
	public function ttlist() {
		...
		$this->load->model('ttocr/ttmodel','tm');

		if ($params['wgroup'] == 'all') {
			foreach ($this->page_config->workgroup as $group) {
				$data['wgroup'][$group]['name'] = $group;
				$data['wgroup'][$group]['data'] = $this->tm->getTTList($group);
				$data['wgroup'][$group]['total'] = $this->tm->ttCount($group);
			}
		} else {
			if (in_array($params['wgroup'], $this->page_config->workgroup)) {
				$data['wgroup'][$params['wgroup']]['name'] = $params['wgroup'];
				$data['wgroup'][$params['wgroup']]['data'] = $this->tm->getTTList($params['wgroup']);
				$data['wgroup'][$params['wgroup']]['total'] = $this->tm->ttCount($params['wgroup']);
			} else {
				$data['wgroup'] = null;
			}
		}

		$this->prepareTemplate('ttlist', $data);
